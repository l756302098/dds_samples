...
static MindosData_Msg data[MAX_SAMPLES];
...
static void data_available(dds_entity_t rd, void *arg)
{
  ...
  for (int j = 0; !dds_triggered (waitSet) && j < samplecount; j++)
  {
    ...
    else if (info[j].valid_data)
    {
      ...
      MindosData_Msg * valid_sample = &data[j];
...
static void finalize_dds(dds_entity_t pp, MindosData_Msg xs[MAX_SAMPLES])
{
  dds_return_t status;
  ...
  {
    MindosData_Msg_free (&xs[i], DDS_FREE_CONTENTS);
  }
}
...
int main (int argc, char ** argv)
{
  ...
  while (!dds_triggered (waitSet))
  {
    ...
    std::cout << "dds_triggered finish." << std::endl;
